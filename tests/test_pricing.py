...
def test_approach_zone_two_percent_above_threshold() -> None:
    assert APPROACH_ZONE_UPPER_FRACTION == Decimal("0.02")
    # порог 100, зона: (100, 102)
    assert is_in_approach_zone(Decimal("101.00"), Decimal("100.00")) is True
    ...
    assert is_in_approach_zone(Decimal("102.00"), Decimal("100.00")) is False
    assert is_in_approach_zone(Decimal("101.99"), Decimal("100.00")) is True
